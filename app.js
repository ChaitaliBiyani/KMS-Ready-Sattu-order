...
document.addEventListener("DOMContentLoaded", () => {
  ...
  // Set default payment fields visibility (UPI selected by default)
  togglePaymentFields();
});
...
function submitOrder() {
  ...
  const methodInput = document.querySelector('input[name="payment-method-choice"]:checked');
  const paymentMethod = methodInput ? methodInput.value : "UPI / Scan QR";
  const upiInput = document.getElementById("payment-upi-id");
...
function processSubmitOrder(orderId, kshetra, custName, custMobile, custEmail, paymentMethod, upiId, activeItems, totalQty, grandTotal) {
  ...
  const upiRadio = document.querySelector('input[name="payment-method-choice"][value="UPI / Scan QR"]');
